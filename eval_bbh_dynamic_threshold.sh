#!/bin/bash
#SBATCH --job-name=eval_bbh_dynamic
#SBATCH --time=1:00:00
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gres=gpu:1
...
accelerate launch --num_processes 1 eval_llada.auto_thresh.py \
  ...
  &> logs/bbh_dynamic_from_global_v2_c${correct_ratio}_mincount${min_count}_minaccepted${min_accepted}_len${length}_block${block}_maxthr${max_threshold}_minthr${min_threshold}.log