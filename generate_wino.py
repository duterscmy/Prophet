...
@torch.no_grad()
def generate_no_log(model, prompt, gen_length=128, block_length=128, temperature=0., mask_id=126336, threshold=0.6, threshold_back=0.9):
...
@torch.no_grad()
def generate(model, prompt, gen_length=128, block_length=128, temperature=0.,
             mask_id=126336, threshold=0.6, threshold_back=0.9):
    ...
    remasked_tokens_total = 0

    x_block = torch.full(
        (1, prompt.shape[1] + gen_length + block_length),
        mask_id,
        dtype=torch.long
    ).to(model.device)
    ...
    for num_block in range(num_blocks):
        ...
        unmask_index_block = torch.full_like(mask_index_block, False)
        unmask_index_block[:, -block_length:] = ~mask_index_block[
            :,
        ...
        attention_mask = torch.ones(
            1, 1, x_block.shape[1], x_block.shape[1],
            dtype=torch.bool
        ).to(device)

        attention_mask[:, :, :, -block_length:] = False
        attention_mask[:, :, -block_length:, -block_length:] = torch.ones(
            block_length, block_length, dtype=torch.bool
        ).to(device)
        ...
        attention_mask[
            ...
            prompt.shape[1] + num_block * block_length:
            prompt.shape[1] + (num_block + 1) * block_length
        ] = ~torch.eye(block_length, dtype=torch.bool).to(device)
        ...
        while mask_index_block.any():
            ...
            unmask_index_block_shift_left = torch.zeros_like(unmask_index_block)
            unmask_index_block_shift_left[
                :,
                prompt.shape[1] + num_block * block_length:
                prompt.shape[1] + (num_block + 1) * block_length
            ] = unmask_index_block[:, -block_length:]
            ...
            # Confidence for masked tokens
            confidence = torch.where(mask_index_block, x0_p, -np.inf)

            # Confidence for previously unmasked back-copy region
            confidence_back = torch.where(unmask_index_block, x0_p, np.inf)

            # =========================
            # Accept / transfer tokens
            # =========================
            transfer_index = confidence > threshold
            ...
            num_accept = int(transfer_index.sum().item())

            if num_accept > 0:
                decoding_steps_used += 1
                accepted_tokens_total += num_accept

            x_block[transfer_index] = x0[transfer_index]

            # =========================
            # Remask low-confidence accepted/back tokens
            # =========================
            if num_accept > 1:
                remask_index = confidence_back < threshold_back
            ...
            remask_index_shift[
                ...
                prompt.shape[1] + num_block * block_length:
                prompt.shape[1] + (num_block + 1) * block_length
            ] = remask_index[:, -block_length:]

            num_remask = int(remask_index_shift.sum().item())
            remasked_tokens_total += num_remask
            ...
            transfer_index_shift = torch.zeros_like(transfer_index)
            transfer_index_shift[:, -block_length:] = transfer_index[
                :,
                prompt.shape[1] + num_block * block_length:
                prompt.shape[1] + (num_block + 1) * block_length
            ]
    ...
    # =========================
    gen_region = output[:, prompt.shape[1]: prompt.shape[1] + gen_length]
    decoded_tokens = int((gen_region != mask_id).sum().item())

    net_decoded_tokens = accepted_tokens_total - remasked_tokens_total
    ...
    print(f"Accepted tokens total: {accepted_tokens_total}")
    print(f"Remasked tokens total: {remasked_tokens_total}")
    print(f"Net decoded tokens: {net_decoded_tokens}")
    print(f"Total block steps: {step}")
    ...
    decoding_stats = {
        ...
        "remasked_tokens_total": remasked_tokens_total,
        "net_decoded_tokens": net_decoded_tokens,
        "total_block_steps": step,
    }

    # Keep original return behavior
    return output
