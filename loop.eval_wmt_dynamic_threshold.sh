#!/bin/bash
#SBATCH --job-name=eval_wmt_de_en_dynamic
#SBATCH --time=3:00:00
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gres=gpu:1
...
for name in "global_v2" "gsm8k_train" "math_train" "ace_train"; do
  threshold_json="token_threshold_on_trainset/${name}_token_threshold_grid_p${correct_ratio}_mincount${min_count}_minaccepted${min_accepted}.json"
  ...
  accelerate launch --num_processes 1 eval_llada.auto_thresh.py \
    ...
    --model llada_dist \
    --num_fewshot 0 \
    --output_path evals_results/auto_thresh/wmt_de_en_dynamic_from_${name}_c${correct_ratio}_mincount${min_count}_minaccepted${min_accepted}_len${length}_block${block}_maxthr${max_threshold}_minthr${min_threshold} \
    --log_samples \
    ...
    &> logs/wmt_de_en_dynamic_from_${name}_c${correct_ratio}_mincount${min_count}_minaccepted${min_accepted}_len${length}_block${block}_maxthr${max_threshold}_minthr${min_threshold}.log
done